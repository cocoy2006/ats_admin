...
public class AdbBackend implements MonkeyRunnerBackend {
    ...
    private static final int CONNECTION_ITERATION_TIMEOUT_MS = 10000;
     ...
     */
    public IDevice findAttacedDevice(String deviceIdRegex) {
    ...
    public AdbMonkeyDevice waitForConnection() {
        return waitForConnection(Integer.MAX_VALUE, ".*");
    }

    public AdbMonkeyDevice waitForConnection(long timeoutMs, String deviceIdRegex) {
        do {
    ...
    public AdbMonkeyDevice getAdbMonkeyDevice(String sn) {
    	if(devices != null && devices.size() > 0) {
    		for(AdbMonkeyDevice mDevice : devices) {
    			if(sn.equals(mDevice.getIDevice().getSerialNumber())) {
    				return mDevice;
    			}
    		}
    	}
    ...
    public void dispose(String sn) {
    	if(devices != null && devices.size() > 0) {
    		int index = -1;
    		for(int i = 0; i < devices.size(); i++) {
    			AdbMonkeyDevice mDevice = devices.get(i);
    			if(sn.equals(mDevice.getIDevice().getSerialNumber())) {
    				mDevice.dispose();
    				index = i;
    				break;
