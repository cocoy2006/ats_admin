	...
	<body>
					  	...
					  	<table class="table table-hover table-bordered">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<a href="<%=basePath%>center/cpts/${disCom.customer.id }/${disCom.project.id }/${disCom.testcase.id }">${disCom.testcase.name}</a>
									</td>
									...
									<td>
										<c:if test="${disCom.dispatcher.state != 9 }">
											<a href="<%=basePath%>cs/report/${disCom.dispatcher.id }" class="text-info">查看</a>|
	    ...
	    function temporarilyDelete(id) {
	    	if(confirm(" 将从系统删除，确定继续?")) {
	    		$.ajax({
	        		url: 'csDispatcher/temporarilyDelete/' + id,
	    ...
	    function permanentlyDelete(id) {
        	if(confirm(" 将从系统彻底删除且无法恢复，确定继续?")) {
        		$.ajax({
            		url: 'csDispatcher/permanentlyDelete/' + id,
            		success: function(data) {
            			location.reload();
            		}
            	});
